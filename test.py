...
model_path = f"模型路径/{model}"

json_path = os.path.join(base_dir,f'{model}-{dataset_name}-test.json')
eval_dataset = load_dataset(f"数据路径/{dataset_name}.json")
...
with open(json_path, mode = 'a', newline='', encoding='utf-8') as file:
    ...
    for ii, ex in enumerate(eval_dataset):
        ...
        sampling_params = SamplingParams(temperature=0.0, max_tokens=max_tokens, ignore_eos = False)
        ...
        for j in range(len(answer)):
            df1, dpr, drecall = scorer_all('dureader_all', extract_after_think(res), str(answer[j]))
            if df1 > temp_df1:
                temp_df1 = df1
            if dpr > temp_dpr:
                temp_dpr = dpr
